...
point new_point(int x0, int y0, int number)
{
    ...
    {
        tmp_point.connections[tmp_com] = -1;
    }
...
void del_all_connections_point(point* tmp_point)
{
    ...
    {
        tmp_point->connections[tmp_con] = 0;
    }
}
void add_connections_ptp(point* tmp_point_1, point* tmp_point_2) // point to point
{
   if (tmp_point_1->number == tmp_point_2->number) { return; }

  for (int tmp_con = 0; tmp_con < tmp_point_1->connections_count; tmp_con++)
  { if ( tmp_point_1->number == tmp_point_2->connections[tmp_con] ) { return; } } 

   tmp_point_1->connections[tmp_point_1->connections_count] = tmp_point_2->number;
  tmp_point_2->connections[tmp_point_2->connections_count] = tmp_point_1->number;
  tmp_point_1->connections_count += 1;
  tmp_point_2->connections_count += 1;
}
...
void check_point_info(point* tmp_point)
{
    ...
    {
        mvprintw( 8, 5 + tmp_check_counter * 5, "%d[%d]\t", tmp_check_counter, tmp_point->connections[tmp_check_counter] );
    }
}