#include "includes.h"
#include "designistions.h"
#include "prototipes.h"

void render_connections_lines(int arr_size, point* tmp_point_arr, bool show_points)
{
    for(int first_counter = 0; first_counter < arr_size; first_counter++)
    {
        ...
        {
            point tmp_p2 = tmp_point_arr[tmp_p1.connections[second_counter]];
...
void render_plane(plane* tmp_plane, bool show_points)
{
    for (int first_counter = 0; first_counter < tmp_plane->points_count; first_counter++)
    {
        ...
        for (int second_counter = 0; second_counter < tmp_p1->connections_count; second_counter++)
        {
            point* tmp_p2 = tmp_plane->point_array[tmp_p1->connections[second_counter]];
    ...
    if (show_points == true)
    {
        ...
        {
            #ifdef DEV_MODE
            mvprintw(tmp_plane->point_array[counter]->y + offset_y, tmp_plane->point_array[counter]->x + offset_x,"(%d)",*tmp_plane->point_array[counter]->number);
            #else
            mvprintw(tmp_plane->point_array[counter]->y + tmp_plane->origin_y, tmp_plane->point_array[counter]->x + tmp_plane->origin_x,POINT_CHAR);
            #endif
        }
    }
}

void render_connection_points(int arr_size, point* tmp_point_arr, int offest_x, int offset_x, int offset_y)
{
