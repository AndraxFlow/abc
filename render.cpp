...
void render_connection_points(int arr_size, point* tmp_point_arr, int offset_x, int offset_y)
{
  ...
  {
    #ifdef DEV_MODE
    mvprintw(tmp_point_arr[counter].y + offset_y, tmp_point_arr[counter].x + offset_x, "(%d)", tmp_point_arr[counter].number);
     #else
...
void render_single_line(point* input_p1, point* input_p2, int offset_x, int offset_y)
{
  point tmp_p1 = *input_p1;
  point tmp_p2 = *input_p2;

  int x0 = tmp_p1.x + offset_x; int tmp_x0 = x0;
  int y0 = tmp_p1.y + offset_y; int tmp_y0 = y0;

  int x1 = tmp_p2.x + offset_x; int tmp_x1 = x1;
